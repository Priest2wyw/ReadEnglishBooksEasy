...
def ReadPDFFile(path):
    ...
    readPDF
    '''
    # pdfFile0bj = open('./datafile/meetingminutes.pdf','rb')
    #  读取文件
    print('*'*20,'Load file','*'*20)
    print('file path:',path)
    pdfReader = PyPDF2.PdfFileReader(path)
    print('pages of file:',pdfReader.numPages) # 查看文件页数
    
    concent = ''
    for i in range(pdfReader.numPages):
        page0bj = pdfReader.getPage(i)
        concent = concent + ' '+page0bj.extractText()
    print('*'*20,'Finish Read PDF File.','*'*20)
    return concent
...
if __name__  ==  "__main__":
    ...
    #     string_add += string
    #     f.close()
    hashmap_sorted = string2words_nums(concent)  
